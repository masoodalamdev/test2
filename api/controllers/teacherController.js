...
class TeacherController {
  static userRegistration = async (req, res) => {
    const { name, email, password, password_confirmation, contact, school, imgUrl, tc } = req.body
    const user = await TeacherModel.findOne({ email: email })
    if (user) {
      res.send({ "status": "failed", "message": "Email already exists" })
    } else {
      if (name && email && password && password_confirmation && contact && school && imgUrl && tc) {
        if (password === password_confirmation) {
          try {
            ...
            await doc.save()
            const saved_user = await TeacherModel.findOne({ email: email })
            // Generate JWT Token
            const token = jwt.sign({ userID: saved_user._id }, process.env.JWT_SECRET_KEY, { expiresIn: '5d' })
            res.status(201).send({ "status": "success", "message": " Registered as Teacher Succesfully", "token": token, "name" : name })
          } catch (error) {
            console.log(error)
  ...
  static changeUserPassword = async (req, res) => {
    ...
    if (password && password_confirmation) {
      if (password !== password_confirmation) {
        ...
        await TeacherModel.findByIdAndUpdate(req.user._id, { $set: { password: newHashPassword } })
    ...
    try {
      ...
      if (password && password_confirmation) {
        if (password !== password_confirmation) {
          ...
          const salt = await bcrypt.genSalt(10)
          const newHashPassword = await bcrypt.hash(password, salt)
          await TeacherModel.findByIdAndUpdate(user._id, { $set: { password: newHashPassword } })
